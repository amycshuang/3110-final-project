...
type block_types = G | W | R | T | H | C | Gym
...
let rec block_type_input file_name w h () =
  ...
  let _ = continue_check_helper (block_type_input file_name w h) in
  ()
...
let rec main () =
  ANSITerminal.(print_string [cyan]
                  "Hi there! To add a map json, the name of the file where this map's json will be stored.\n"); 
